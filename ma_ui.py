...
class UIManager:
    ...
    def build_ui(self, llm):
        ...
        with gr.Blocks(title="科研小助手", theme=gr.themes.Soft(), analytics_enabled=False, css=css) as demo:
            self.prj_name_tb = gr.Textbox(value=f'{os.environ["PRJ_DIR"]}', visible=False)  # 没有实际含义
            ...
            with gr.Accordion('对话管理', open=False):
                with gr.Row():
                    self.conversation_list = gr.Dropdown(label='对话列表', choices=[], container=False, scale=5)
                    new_conversation_btn = gr.Button('新建对话', variant='primary', scale=1, min_width=100)
                with gr.Row():
                    self.conversation_history = gr.Chatbot(label='对话历史', elem_id='prg_chatbot')
            ...
            def upload_file_handler(file, user_id):
                ...
                # 更新前端数据，把新的资源选项加上
                self.update_resource_choices(user_id)
            ...
            self.conversation_list.change(fn=lambda conversation_id: select_conversation(conversation_id), inputs=[self.conversation_list], outputs=[self.conversation_history])
            new_conversation_btn.click(fn=lambda: create_new_conversation(self.user_id), inputs=[], outputs=[self.conversation_list, self.conversation_history])
            prj_chat_btn.click(fn=lambda message: send_message(self.user_id, self.current_conversation_id, message), inputs=[prj_chat_txt], outputs=[self.conversation_history])
