#file:/Users/zyb/Desktop/CSU_Zichen/graduation- design/Ashes_reader/gr_funcs.py
import re
import time
import json
import gradio as gr
...
def view_prj_file(selected_file):
    ...
    if selected_file.endswith('.py'):
        yield gr.update(visible=True, language='python'), gpt_label_update, gpt_res_update
    elif selected_file.endswith('.json'):
        yield gr.update(visible=True, language='json'), gpt_label_update, gpt_res_update
    else:
        yield gr.update(visible=True, language=None), gpt_label_update, gpt_res_update
...
def github_search_func(query):
    ...
    results, paper_choices = github_search.search_github(query, dir_path, max_results=5)
    return results, paper_choices
...
def process_github_repo(selected_repo):
    ...
    if github_search.download_repo(owner, repo, dir_path):
        return f"仓库 {selected_repo} 下载成功，保存在 {dir_path}"
    else:
        return f"下载仓库 {selected_repo} 失败"