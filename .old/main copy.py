#file:/Users/zyb/Desktop/CSU_Zichen/graduation- design/Ashes_reader/main.py
import os
import time
...
import config
import gr_funcs
import utils.arXiv_search as arXiv_search  # 导入 arXiv_search 模块
...
def main(prj_dir):
    ...
    with gr.Blocks(title="科研小助手", theme=gr.themes.Soft(), analytics_enabled=False, css=css) as demo:
        ...
        with gr.Accordion(label='论文搜索', open=False):
            with gr.Row():
                search_query = gr.Textbox(label='搜索查询', placeholder='请输入论文序列号、关键词或作者', container=False, scale=5)
                search_btn = gr.Button(value='搜索', variant='primary', scale=1, min_width=100)
            with gr.Row():
                search_results = gr.Markdown(label='搜索结果', elem_classes='markdown-class')
            with gr.Row():
                selected_paper = gr.Dropdown(label='选择论文', choices=[], container=False, scale=5)
                process_paper_btn = gr.Button(value='处理论文', variant='primary', scale=1, min_width=100)
            with gr.Row():
                paper_summary = gr.Markdown(label='论文摘要', elem_classes='markdown-class')

        # 新增的 GitHub 搜索选项卡
        with gr.Accordion(label='GitHub 搜索', open=False):
            with gr.Row():
                github_query = gr.Textbox(label='搜索查询', placeholder='请输入 GitHub 仓库名称或关键词', container=False, scale=5)
                github_search_btn = gr.Button(value='搜索', variant='primary', scale=1, min_width=100)
            with gr.Row():
                github_search_results = gr.Markdown(label='搜索结果', elem_classes='markdown-class')
            with gr.Row():
                selected_repo = gr.Dropdown(label='选择仓库', choices=[], container=False, scale=5)
                process_repo_btn = gr.Button(value='处理仓库', variant='primary', scale=1, min_width=100)
            with gr.Row():
                repo_summary = gr.Markdown(label='仓库摘要', elem_classes='markdown-class')
        ...
        # 论文搜索功能
        search_btn.click(gr_funcs.arxiv_search_func, inputs=[search_query], outputs=[search_results, selected_paper])
        process_paper_btn.click(gr_funcs.process_paper, inputs=[selected_paper], outputs=[paper_summary])

        # GitHub 搜索功能
        github_search_btn.click(gr_funcs.github_search_func, inputs=[github_query], outputs=[github_search_results, selected_repo])
        process_repo_btn.click(gr_funcs.process_github_repo, inputs=[selected_repo], outputs=[repo_summary])
